...
func TestHandlerRendersRideDetailWithEmbeddedRoute(t *testing.T) {
	...
	assert.NotEmpty(t, profileScript)

	logicStaticRequest := httptest.NewRequest(http.MethodGet, "/static/official-climb-profile-logic.js", nil)
	logicStaticResponse := httptest.NewRecorder()
	server.Handler().ServeHTTP(logicStaticResponse, logicStaticRequest)
